...
{% if not goodreads_id is sameas none}<span class="book_desc_field">Goodreads</span>: <span class="book_desc_entry"><a href="https://www.goodreads.com/book/show/{{books.goodreads_id}}">{{books.goodreads_id}}</a></span><br/>{% endif %}
{% for field_name, entry_field in [("Publication Date", pub_date"), ("Original Publication Date", "original_pub_date"), ("Duration", "duration"), ("Pages", "pages") %}
    {%if not book[entry_field] is sameas none %}
    <span class="book_desc_field">{{ field_name }}</span>: <span class="book_desc_entry">{{ book[entry_field] }}</span><br/>
    {% endif %}
{{description}}
{%endfor%}
